#!/bin/bash
#
# This script is for setting up a debian environment for building
# OpenXT.
#
...
# Copyright (c) 2016 Apertus Solutions
#
...
PKGS="openssh-server openssl sed wget cvs subversion git-core coreutils unzip \
	texi2html texinfo docbook-utils gawk python-pysqlite2 diffstat help2man \
	make gcc build-essential g++ desktop-file-utils chrpath cpio guilt iasl \
	quilt bin86 bcc libsdl1.2-dev liburi-perl genisoimage policycoreutils \
	unzip vim sudo rpm curl libncurses5-dev man file"

apt-get update

for p in $PKGS; do
	apt-get --no-install-recommends --force-yes -y -q install $p
done
